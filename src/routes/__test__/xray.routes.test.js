...
describe("POST /api/xrays", () => {
  ...
  describe("When info is missing", () => {
    test("Should respond with a 422 status code", async () => {
      ...
      const res = await request(app).post("/api/login").send(staffBody)
      const token = res.body.token
      const response = await request(app).post("/api/xrays").set("token", token)
...
describe("PUT /api/xrays/:id", () => {
  describe("When info is correct", () => {
    test("Should respond with a 201 status code", async () => {
      ...
      const res = await request(app).post("/api/login").send(staffBody)
      const token = res.body.token
      const response = await request(app).put("/api/xrays/1").set("token", token).send(bodyData)

      expect(response.statusCode).toBe(201)
      expect(response.headers['content-type']).toEqual(expect.stringContaining('json'))
      expect(response.body.body).toEqual(userPayload);
      expect(response.body.message).toEqual("X-ray Updated Successfully");
    });
  });

  describe("When user is not authenticated", () => {
    test("Should respond with a 403 status code", async () => {
      const response = await request(app).put("/api/xrays/1")
  ...
  describe("When info is missing", () => {
    test("Should respond with a 422 status code", async () => {
      ...
      const res = await request(app).post("/api/login").send(staffBody)
      const token = res.body.token
      const response = await request(app).put("/api/xrays/1").set("token", token)
